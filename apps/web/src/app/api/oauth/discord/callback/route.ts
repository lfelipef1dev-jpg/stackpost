...
import { oauth_discord_callbackQuerySchema } from '@/lib/schemas';
...
export async function GET(req: NextRequest) {
  ...
  const parsedQuery = oauth_discord_callbackQuerySchema.safeParse(queryRaw);
  ...
  if (!clientId || !clientSecret) return NextResponse.json({ error: 'Discord OAuth não configurado' }, { status: 500 });
