  ...
  try {
    ...
    if (existing) {
      const { error: updateError } = await supabase
        .from('social_accounts')
        .update({ access_token: data.accessToken, external_id: data.instagramId, expires_at: data.expiresAt })
        .eq('id', existing.id);
    ...
    return NextResponse.redirect(new URL('/dashboard?connected=instagram', req.url));
  } catch (error: any) {
