...
import { oauth_youtube_callbackQuerySchema } from '@/lib/schemas';
...
export async function GET(req: NextRequest) {
  ...
  const parsedQuery = oauth_youtube_callbackQuerySchema.safeParse(queryRaw);
  if (!parsedQuery.success) return NextResponse.json({ error: parsedQuery.error.issues }, { status: 400 });
  const code = searchParams.get('code');
  const state = searchParams.get('state');
  const storedState = req.cookies.get('oauth_state_youtube')?.value;
  ...
  const config = OAUTH_CONFIGS['youtube'];
  try {
    const tokenData = await exchangeCodeForToken(config, code);
    await saveAccount(req, 'youtube', tokenData, {
      username: 'youtube_user',
      externalId: tokenData.raw?.user?.open_id || tokenData.raw?.user_id,
    });
    return NextResponse.redirect(new URL('/dashboard?connected=youtube', req.url));
  } catch (err: any) {
