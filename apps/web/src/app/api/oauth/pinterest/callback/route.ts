...
import { oauth_pinterest_callbackQuerySchema } from '@/lib/schemas';
...
export async function GET(req: NextRequest) {
  ...
  const parsedQuery = oauth_pinterest_callbackQuerySchema.safeParse(queryRaw);
  if (!parsedQuery.success) return NextResponse.json({ error: parsedQuery.error.issues }, { status: 400 });
  const code = searchParams.get('code');
  const state = searchParams.get('state');
  const storedState = req.cookies.get('oauth_state_pinterest')?.value;
  ...
  try {
    ...
    await saveAccount(req, 'pinterest', tokenData, {
      username: 'pinterest_user',
      externalId: tokenData.raw?.user?.open_id || tokenData.raw?.user_id,
    });
    return NextResponse.redirect(new URL('/dashboard?connected=pinterest', req.url));
  } catch (err: any) {
    logger.error('pinterest OAuth error:', err);
