...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setError(formatError(data.error) || 'Erro ao entrar');
    }
  }
      ...
      <div className="w-full max-w-md p-8 rounded-2xl bg-brand-surface border border-brand-border">
        <div className="text-center space-y-2 mb-8">
          ...
          <p className="text-brand-text-secondary text-sm">
            Faca login com seu email e senha.
          </p>
        </div>
        ...
        <p className="text-center text-brand-text-secondary text-sm mt-6">
          Ainda nao tem conta? <a href="/register" className="text-brand-accent hover:underline">Criar</a>
        </p>
      </div>
    </main>
  );
