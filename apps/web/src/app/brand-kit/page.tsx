...
export const metadata: Metadata = { title: 'Brand Kit - Logo, cores e assets do StackPost', description: 'Brand kit do StackPost: logo, paleta de cores, tipografia e assets para parceiros e integradores.', alternates: { canonical: '/brand-kit' } };
      ...
      <section className="relative pt-32 pb-16 px-4 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative">
          <FadeIn>
            ...
            <p className="text-lg text-brand-text-secondary">Logo, cores e assets do StackPost.</p>
          </FadeIn>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-20 space-y-12">
        ...
        <ScrollReveal>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="p-8 rounded-xl bg-brand-surface/50 backdrop-blur border border-brand-border flex flex-col items-center gap-4">
              ...
              <p className="text-xs text-brand-text-secondary">Logo principal (dark)</p>
            </div>
            <div className="p-8 rounded-xl bg-white border border-brand-border flex flex-col items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 rounded-lg bg-[#8AB4F8] flex items-center justify-center">
                  <span className="text-white font-bold">S</span>
                </div>
                <span className="text-2xl font-bold text-black">StackPost</span>
              </div>
              <p className="text-xs text-gray-500">Logo principal (light)</p>
            </div>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { name: 'brand-bg', hex: '#0A0A0A', desc: 'Background' },
              { name: 'brand-surface', hex: '#1A1A1A', desc: 'Surface' },
              { name: 'brand-accent', hex: '#8AB4F8', desc: 'Accent' },
              { name: 'brand-text', hex: '#E6E6E6', desc: 'Text' },
            ].map((c) => (
              <div key={c.name} className="p-4 rounded-xl bg-brand-surface/50 backdrop-blur border border-brand-border">
                <div className="w-full h-16 rounded-lg mb-3" style={{ backgroundColor: c.hex }} />
                <p className="font-mono text-sm text-brand-text">{c.hex}</p>
                <p className="text-xs text-brand-text-secondary">{c.name} - {c.desc}</p>
              </div>
            ))}
          </div>
        </ScrollReveal>
